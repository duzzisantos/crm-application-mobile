...
class StockQuotes(MDScreen):
    def __init__(self, sm, **kwargs):
        ...
        innerLayout = BoxLayout(orientation="horizontal", padding=[40, 80])
    ...
    def handle_submit(self, instance):
        ...
        try:
            ...
            stock_table = MDDataTable(
                row_data=stock_row_data, column_data=stock_column_headers, elevation=2
            )

            table_wrapper = BoxLayout(orientation="vertical", padding=[20, 80])
            backToQuotes = Button(text="<< Back to Quotes", font_size=30)
            table_wrapper.add_widget(backToQuotes)
            table_wrapper.add_widget(stock_table)
            self.add_widget(table_wrapper)
