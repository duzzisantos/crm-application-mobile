    ...
    def __init__(self, sm, **kwargs):
        ...
        try:
            ...
            for each in segmentation_by_region:
                ...
                if selected_region not in state_segmentation:
                    state_segmentation[selected_region] = {
                        "state": set(),
                        "total_customer_size": 0,
                    }

                ## increment state count and add aggregate customer size per region
                state_segmentation[selected_region]["state"].add(selected_state)
                state_segmentation[selected_region][
                    "total_customer_size"
                ] += selected_customer_size

            table_row_data = [
                (values[len(values["state"])], values["total_customer_size"])
                for values in state_segmentation.items()
            ]
