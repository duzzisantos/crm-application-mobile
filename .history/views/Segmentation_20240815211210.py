...
from utilities import usRegions
...
class Segmentation(MDScreen):
    def __init__(self, sm, **kwargs):
        ...
        try:
            ...
            offical_regions = [usRegions.us_regions.keys()]
            for customer in clients:
                for region in usRegions.us_regions:
                    for state in region:
                        if customer["usState"].__eq__(state):
                            segmentation_by_region.append(
                                {
                                    "region": region,
                                    "us_state": customer["usState"],
                                    "client_population": region.count(
                                        customer["usState"]
                                    ),
                                }
                            )
