...
class AccountManagers(MDScreen, RecycleView):
    def __init__(self, sm, **kwargs):
        ...
        refreshBtn = Button(
            text="Refresh", font_size=25, size_hint=(None, None), size=(dp(100), dp(40))
        )

        refreshBtn.bind(on_press=self.refresh_api)
        layout.add_widget(refreshBtn)
        ...
        try:
            ...
            data_dict = xmltodict.parse(
                accountManagerList.content or self.refreshed_data.content
            )
    ...
    def refresh_api(self, instance):
        try:
            refreshed = requests.get(
                "http://localhost:8080/CRMApplication/webresources/entity.accountmanagers"
            )

            self.refreshed_data = refreshed
            return self.refreshed_data
        except:
            print("Error in refetching data")

    def delete_manager(self, id):
        print("Deleted: " + id)
        ##This is where we make the API call that deletes a resource by ID


class MyApp(MDApp):
    def build(self):
        sm = ScreenManager()
        Window.title = "Account Managers"
